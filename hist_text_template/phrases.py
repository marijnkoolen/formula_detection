...
def find_sub_phrase_extensions(curr_node, transition_probs, min_split_prob: float = 0.1,
    ...
    extended_phrases = {curr_node: curr_type}
    ...
    for extended_node in transition_probs[curr_node]:
        ...
        if extended_prob >= min_extend_prob:
            ...
            extended_phrases[extended_node] = 'main'
            # print('find_sub_phrase_extensions start - setting main:', extended_node)
            del extended_phrases[curr_node]
            # print('find_sub_phrase_extensions start - deleting:', curr_node)
        if extended_prob >= min_split_prob:
            extra_phrases = find_sub_phrase_extensions(extended_node, transition_probs, min_split_prob=min_split_prob,
                                                       min_extend_prob=min_extend_prob, curr_prob=extended_prob)
            for extended_phrase in extra_phrases:
                # print('find_sub_phrase_extensions start - setting type:', extended_phrase, extra_phrases[extended_phrase])
                if extra_phrases[extended_phrase] == 'main':
                    if extended_phrases[extended_node] != 'main':
                        print('phrase:', extended_node, extended_phrases[extended_node])
                        print('extension:', extended_phrase, extra_phrases[extended_phrase])
                        raise TypeError('extension of phrase is main but phrase itself is not')
                    # print('find_sub_phrase_extensions start - replacing main:', extended_node, extended_phrase, extra_phrases[extended_phrase])
                    del extended_phrases[extended_node]
                extended_phrases[extended_phrase] = extra_phrases[extended_phrase]
                if curr_node in extended_phrases and curr_prob < min_extend_prob:
                    del extended_phrases[curr_node]
            # print('\tSPLITTING')
...
def get_extended_phrases(sub_phrase, pre_extended_sub_phrases, post_extended_sub_phrases):
    main_phrase = sub_phrase
    split_phrases = []

    for pre_extended_sub_phrase in pre_extended_sub_phrases:
        if pre_extended_sub_phrases[pre_extended_sub_phrase] == 'main':
    ...
    for pre_extended_sub_phrase in pre_extended_sub_phrases:
        if pre_extended_sub_phrases[pre_extended_sub_phrase] == 'split':
            split_phrase = ' '.join(reversed(pre_extended_sub_phrase))
            split_phrases.append(split_phrase)

    main_elements = 1
    for post_extended_sub_phrase in post_extended_sub_phrases:
        if post_extended_sub_phrases[post_extended_sub_phrase] == 'main':
    ...
    for post_extended_sub_phrase in post_extended_sub_phrases:
        if post_extended_sub_phrases[post_extended_sub_phrase] == 'split':
